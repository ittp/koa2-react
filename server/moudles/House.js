...
class House extends Base {
  constructor(obj = {}) {
    ...
    // 售价
    this.sellPrice = obj.sellPrice || 100;
    // 租金
    this.rentPrice = obj.rentPrice || 1000;
    // 租金类型 0:月租,1:季租,2:半年租,3:整年租,4:多年租
    this.rentType = obj.rentType || 0;
    // 大小
    this.square = obj.square || 0;
    // 楼层
    this.floor = obj.floor || 1;
    // 总楼层
    this.totalFloor = obj.totalFloor || 1;
    // 卧室
    this.bedroom = obj.bedroom || 1;
    // 客厅
    this.drawingroom = obj.drawingroom || 1;
    // 卫生间
    this.toilet = obj.toilet || 1;
    // 装修 0:豪装,1:精装,2:中装,3:简装,4:毛坯
    this.decorate = obj.decorate || 1;
    // 类型 0:主卧,1:次卧,2:隔断,3:整租
    this.roomtype = obj.roomtype || 0;
    // 朝向
    this.orientation = obj.orientation || '无';
    // 限制 0:男,1:女,2:男女不限
    ...
    this.tag = obj.tag || [];
    // 照片
    this.imgs = obj.imgs || ['http://dongkunshan.cn/imgs/house.png'];
    // 状态 0:有效,1:无效
  ...
  static async list(opt) {
    ...
    try {
      // get by query
      const db = new Mongodb(connectionStr, 'house', true);
      ret.count = await db.count();
      ret.list = await db.find({}, {}, (opt.pageNum - 1) * opt.pageSize, opt.pageSize, {
        _id: -1
      });
    }
    catch (e) {
